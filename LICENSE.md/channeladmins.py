...
async def pause(_, message: Message):
    ...
    if (chat_id not in callsmusic.active_chats) or (
        ...
        await message.reply_text("❗ Nothing is playing LOL !")
    else:
...
async def stop(_, message: Message):
    ...
    if chat_id not in callsmusic.active_chats:
        await message.reply_text("❗ Nothing is streaming Mad!")
    else:
        try:
        ...
        await message.reply_text("❌ Stopped streaming (Thanks)!")
...
async def skip(_, message: Message):
    ...
    chat_id = chid
    if chat_id not in callsmusic.active_chats:
        await message.reply_text("❗ Nothing is playing to skip(pagal ladka / Ladki )!")
    else:
...
async def admincache(client, message: Message):
    ...
    await message.reply_text("❇LOL Hogaya Admin cache refreshed!")
